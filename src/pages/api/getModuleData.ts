// an api route to fetch a course module and return the correct badge type: New or Recently Updated
...
export default async function getModules(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!MODULE.toString().match(/^\/courses\/[^/]+\/[^/]+$/))
        return res.status(400).json({ error: '`module` must be in the form /courses/course-name/module-name' })
