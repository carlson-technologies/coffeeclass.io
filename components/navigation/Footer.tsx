...
import { year } from '../../lib/scripts/get-year'
...
const FooterNavItem = ({ text, href }: PropsFooterNavItem) => {
  return (
    <Text textAlign="left">
      <Link as={NextLink} href={href} _hover={{ textDecor: 'underline' }}>
        {text}
      </Link>
    </Text>
  )
    ...
    <Box bgColor={useColorModeValue('gray.100', 'gray.700')}>
      ...
      >
        {!error && (
          <Wrap maxW={1200} justify="center" mx="auto">
            {data?.tags.map((tag: string, index: number) => (
              <Link key={index} as={NextLink} href={`/tags/${tag}`}>
                <Badge
        ...
        <Text as="small" mt={4}>
          ...
          <Link href="https://carlsontechnologies.dev/" textDecor="underline" isExternal>
